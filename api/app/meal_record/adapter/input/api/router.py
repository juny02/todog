...
from app.meal_record.adapter.input.api.request.GetMealRecordsRequest import GetMealRecordsRequest
...
from app.meal_record.application.usecase.CreateMealRecordUseCase import (
    CreateMealRecordUseCase,
)
from app.meal_record.application.usecase.DeleteMealRecordUseCase import (
    DeleteMealRecordUseCase,
)
from app.meal_record.application.usecase.GetMealRecordsUseCase import (
    GetMealRecordsUseCase,
)
from app.meal_record.application.usecase.GetMealRecordUseCase import (
    GetMealRecordUseCase,
)
from app.meal_record.application.usecase.UpdateMealRecordUseCase import (
...
async def get_meal_records_by_dog(
    *,
    dog_id: str,
    query: GetMealRecordsRequest = Depends(),
    get_meal_records: GetMealRecordsUseCase = Depends()
):
    cmd = GetMealRecordsCommand(dog_id=dog_id, **query.model_dump())
    return await get_meal_records(cmd)


@router.get(
    "/{dog_id}/meal_records/{meal_record_id}", response_model=GetMealRecordResponse
)
...
@router.delete(
    "/{dog_id}/meal_records/{meal_record_id}", status_code=status.HTTP_204_NO_CONTENT
)
